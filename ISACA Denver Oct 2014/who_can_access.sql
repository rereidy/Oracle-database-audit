...
--                                   output.
-- R. Reidy    1.4         Dec 2013  Removed all references to utl_file and the need to create a directory object
-- -----------------------------------------------------------------------------
    ...
    begin
        ...
        for lv_main in c_main(pv_object,pv_owner) loop
            ...
            if lv_userrole='USER' then
                write_op(chr(9)||'User => '||lv_main.grantee ||' (ADM = '||lv_main.grantable||')');
            else
                if lv_main.grantee='PUBLIC' then
                    write_op(chr(9)||'Role => '||lv_main.grantee ||' (ADM = '||lv_main.grantable||')');                    
                else
                    write_op(chr(9)||'Role => '||lv_main.grantee ||' (ADM = '||lv_main.grantable||')' ||' which is granted to =>');
